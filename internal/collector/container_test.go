...
func TestContainerCollector_Collect_Success(t *testing.T) {
	...
	mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasSuffix(r.URL.Path, "/containers/json") {
			...
			w.Write([]byte(`[{"Id":"test-cont-id","Names":["/test-container"],"Image":"test-image"}]`))
			return
		}

		// Mock container stats - Resolvido utilizando a lógica da main (ID correto)
		if strings.HasSuffix(r.URL.Path, "/containers/test-cont-id/stats") {
			w.Header().Set("Content-Type", "application/json")
			statsJSON := `{
	...
	m := metrics[0]
	assert.Equal(t, "test-cont-id", m.ContainerID)
